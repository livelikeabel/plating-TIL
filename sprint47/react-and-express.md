...
컴포넌트가 돔위에 생성되기 전과 후, 그리고 데이터가 변경되어 상태를 업데이트 하기 전과 후, 그 다음에 마지막으로 컴포넌트가 돔에서 사라지기 전에 실행되는 메소드들 입니다.

Life cycle api 는 총 7개가 있다.

- componentWillMount // 렌더링이 되기 전에 실행됨
- componentDidMount // 렌더링이 된 다음
- componentWillReceiveProps // 새로운 props를 받았을 때
- shouldComponentUpdate // 컴포넌트가 업데이트를 할지말지 정하는 부분
- componentWillUpdate // 컴포넌트가 업데이트 되기 전에 실행됨
- componentDidUpdate // 컴포넌트가 업데이트 된 다음에 실행됨
- componentWillUnmount // 컴포넌트가 제거 될 때



컴포넌트가 생길 때에는 컴포넌트 api들이 아래의 순서대로 실행된다.

constructor -> componentWillMount -> render -> componentDidMount

Unmount할 때는 componentWillUnmount만 실행된다.



#### **component의 중요 메소드 살펴보기**
...
> 컴포넌트가 DOM 위에 만들어지기 전에 실행된다.(그래서 여기서는 DOM처리를 할 수 없다.)
...
> 첫 렌더링을 마치고 실행된다. 이 안에서 다른 자바스크립트 프레임워크 연동 및 setTimeout, setInterval 및 AJAX 사용
...
> props를 받을 때 실행된다. props에 따라 state를 업데이트 할 때 사용하면 유용하다. 이 안에서 setState를 해도 괜찮다.
...
> 컴포넌트 업데이트 전 실행된다
>
> 여기서 setState 절대 사용하지 말 것(무한루프 걸린다.)
